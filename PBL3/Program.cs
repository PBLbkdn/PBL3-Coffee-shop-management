...
namespace PBL3
{
    internal static class Program
    {
        ...
        private static void OnTimedEvent(Object source, ElapsedEventArgs e)
        {
            ...
            {
                CaTruc_BLL.Instance.TinhDoanhThuCa(endTimeCa1.ToString("MM-dd-yyyy"), 1);
            }
            if (DateTime.Now >= endTimeCa2)
            {
                CaTruc_BLL.Instance.TinhDoanhThuCa(endTimeCa2.ToString("MM-dd-yyyy"), 2);
            }
            if (DateTime.Now >= endTimeCa3)
            {
                CaTruc_BLL.Instance.TinhDoanhThuCa(endTimeCa3.ToString("MM-dd-yyyy"), 3);
            }
